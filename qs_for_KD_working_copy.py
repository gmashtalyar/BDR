...
def formatting_KD(YYYYYY, XXXXXXXX):
    ...
    data_divisions['Октябрь 2022'] = data_divisions['Октябрь 2022'].apply(abs)
    data_divisions['Ноябрь 2022'] = data_divisions['Ноябрь 2022'].apply(abs)
    data_divisions['Декабрь 2022'] = data_divisions['Декабрь 2022'].apply(abs)
    ...
    tons_10 = total_tons_sold['October'].sum()
    tons_11 = total_tons_sold['November'].sum()
    tons_11 = total_tons_sold['December'].sum()
    ...
    data_divisions['Октябрь 2022 Т'] = data_divisions_tons['Октябрь 2022'].div(tons_10).round(2).apply(abs)
    data_divisions['Ноябрь 2022 Т'] = data_divisions_tons['Ноябрь 2022'].div(tons_11).round(2).apply(abs)
    data_divisions['Декабрь 2022 Т'] = data_divisions_tons['Декабрь 2022'].div(tons_11).round(2).apply(abs)
    ...
    data_melted_1 = data_divisions.melt(id_vars=[
        ...
        'Сентябрь 2022', 'Октябрь 2022', 'Ноябрь 2022', 'Декабрь 2022'])
    ...
    data_melted_2 = data_divisions.melt(id_vars=[
        ...
        'Август 2022 Т', 'Сентябрь 2022 Т', 'Октябрь 2022 Т', 'Ноябрь 2022 Т', 'Декабрь 2022 Т'])
    ...
    data_melted_1['Expense 1'] = data_melted_1['Expense 1'].replace([
        'Услуги аутсорсеров (складские)', 'Услуги СК ВАЛ (складские)'], 'Складские расходы')
    data_melted_1['Expense 1'] = data_melted_1['Expense 1'].replace(['Проценты'], 'Финансирование')
    data_melted_1['Expense 1'] = data_melted_1['Expense 1'].replace(['Услуги ППЖТ'], 'ППЖТ')


    data_melted_1['Номер строки'] = ''
    data_melted_1.loc[(data_melted_1['Статья бюджета'] == 'Резерв ДЗ'), 'Expense 1'] = 'Списание ДЗ'
    data_melted_1.loc[(data_melted_1['Статья бюджета'] == 'Проценты на выделенные активы'), 'Expense 1'] = 'Финансирование'
    data_melted_1.loc[(data_melted_1['Статья бюджета'] == 'Проценты на активы (залог для тендеров)'), 'Expense 1'] = 'Финансирование'
    data_melted_1.loc[(data_melted_1['Статья бюджета'] == 'Проценты долгосрочный кредит'), 'Expense 1'] = 'Финансирование'

    data_melted_1.loc[(data_melted_1['Expense 1'] == 'ФОТ'), 'Номер строки'] = 1
    data_melted_1.loc[(data_melted_1['Expense 1'] == 'ЕСН'), 'Номер строки'] = 2
    data_melted_1.loc[(data_melted_1['Expense 1'] == 'Прочие расходы на персонал'), 'Номер строки'] = 3
    data_melted_1.loc[(data_melted_1['Expense 1'] == 'Складские расходы'), 'Номер строки'] = 4
    data_melted_1.loc[(data_melted_1['Expense 1'] == 'ППЖТ'), 'Номер строки'] = 5
    data_melted_1.loc[(data_melted_1['Expense 1'] == 'Материальные затраты'), 'Номер строки'] = 6
    data_melted_1.loc[(data_melted_1['Expense 1'] == 'Амортизация'), 'Номер строки'] = 7
    data_melted_1.loc[(data_melted_1['Expense 1'] == 'Финансирование'), 'Номер строки'] = 8
    data_melted_1.loc[(data_melted_1['Expense 1'] == 'Списание ДЗ'), 'Номер строки'] = 9
    data_melted_1.loc[(data_melted_1['Expense 1'] == 'Услуги ВГО'), 'Номер строки'] = 10
    data_melted_1.loc[(data_melted_1['Expense 1'] == 'прочее (вычеты со знаком минус)'), 'Номер строки'] = 11
    data_melted_1.loc[(data_melted_1['Expense 1'] == 'Прочие налоги уплаченные'), 'Номер строки'] = 12
    data_melted_1.loc[(data_melted_1['Expense 1'] == ' Налог на прибыль'), 'Номер строки'] = 13

    data_melted_1 = data_melted_1.loc[data_melted_1['Статья бюджета'] != 'Транспорт (доп.расходы к реализациям)']
    data_melted_1 = data_melted_1.loc[data_melted_1['Expense 1'] != 'Компенсация ППЖТ ХРАНИТЕЛЯМИ без НДС']
...
formatting_KD('ВАЛ', 'VAL')
formatting_KD('АМД', 'AMD')
formatting_KD('АТМД', 'ATMD')