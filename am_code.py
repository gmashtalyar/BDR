...
def pasting_am(i, y):
    ...
    data_am.to_excel('data_output/data_am30.xlsx')
    ...
    data_am_sales = data_am_sales.loc[data_am_sales['Company'] == 'Ариэль Металл']
    ...
    data_metally_sales = data_metally_sales.loc[data_metally_sales['Отдел'] != 'КД МСК']
    data_metally_sales = data_metally_sales.loc[data_metally_sales['Отдел'] != 'КД СПБ']
    data_metally_sales = data_metally_sales.loc[data_metally_sales['Отдел'] != 'КД Проекты']
    data_metally_sales_OS = data_metally_sales.loc[data_metally_sales['Expense type_'] == 'продажа ОС и иных активов']

    data_am_sales.to_excel('data_output/data_am_sales.xlsx')
    data_metally_sales.to_excel('data_output/data_metally_sales.xlsx')
    data_metally_sales_OS.to_excel('data_output/data_metally_sales_OS.xlsx')
    ...
    expenses_am.to_excel('data_output/expenses_am.xlsx')
    expenses_am = expenses_am.groupby(['Expense 1', 'Статья бюджета']).sum().reset_index()
    expenses_am.to_excel('data_output/expenses_am2.xlsx')
    ...
    summa_metally_sales_os = data_metally_sales_OS.groupby(['Division', 'Counter Party']).sum().reset_index()

    bdr_sales_amMetally_OfficeLease = summa_metally_sales.loc[summa_am_sales['Expense type_'] == 'Аренда офиса (доход)']
    bdr_sales_amMetally_Marketing = summa_metally_sales.loc[
        summa_am_sales['Expense type_'] == 'Компенсация расходов по интернет-проектам (доход)']
    bdr_sales_amMetally_Tax1 = summa_metally_sales.loc[summa_am_sales['Expense type_'] == 'Налог в УК']
    bdr_sales_amMetally_Tax2 = summa_metally_sales.loc[summa_am_sales['Expense type_'] == 'Налог в УК (доход)']
    bdr_sales_amMetally_OtherServices = summa_metally_sales.loc[
        summa_am_sales['Expense type_'] == 'Услуги прочие (доход)']
    bdr_sales_amMetally_AntiCor = summa_metally_sales.loc[summa_am_sales['Expense type_'] == 'Антикор Полимер (прибыль)']
    bdr_sales_amMetally_SKTK = summa_metally_sales.loc[summa_am_sales['Expense type_'] == 'ППУ (прибыль)']
    bdr_sales_amMetally_InterestReceived = summa_metally_sales.loc[summa_metally_sales['Expense type_'] == 'Процент по займам']
    bdr_sales_amMetally_BadDebitReturn = summa_metally_sales.loc[summa_am_sales['Expense type_'] == 'Списание задолжности (доход)']
    ...
    expense_check = expenses_am_FOT[y].sum() + expenses_am_ESN[y].sum() + expenses_am_Personnel[y].sum() +\
                    ...
                    expenses_am_bdrincometax[y].sum()
    ...
    sheet_am[f'{i}186'] = bdr_sales_am_BadDebitReturn[y].sum()/1000 + bdr_sales_amMetally_BadDebitReturn[y].sum()/1000  # списадие ДЗ
    ...
    a3 = len(bdr_sales_am_LatePayments) + shift
    ...
    for j in range(len(bdr_sales_am_LatePayments)):
        sheet_am[f'{i}{transcript_cell+j}'] = bdr_sales_am_LatePayments.iloc[j][y]/1000
        sheet_am[f'A{transcript_cell+j}'] = bdr_sales_am_LatePayments.iat[j, 0]
        sheet_am[f'B{transcript_cell+j}'] = bdr_sales_am_LatePayments.iat[j, 1]
    ...
    for j in range(len(expenses_am_Unknown)):
        sheet_am[f'{i}{transcript_cell+j+a7+shift}'] = expenses_am_Unknown.iloc[j][y]/1000
        sheet_am[f'A{transcript_cell+j+a7+shift}'] = expenses_am_Unknown.iat[j, 0]
        sheet_am[f'B{transcript_cell+j+a7+shift}'] = expenses_am_Unknown.iat[j, 1]
